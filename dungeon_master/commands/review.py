...
def run_review(mark_reviewed=None):
    ...
    try:
        ...
        if mark_reviewed:
            ...
            console.print("  • You can confidently confirm documentation remains accurate")
            console.print()
        ...
        # Create status table
        table = Table(title="📚 Documentation Status", show_header=True, header_style="bold blue")
        ...
        for lore_file_path, tracked_files in mapping.items():
            ...
            if not full_path.exists():
                ...
                issues_found.append((lore_file_path, "CREATE", f"Run: dm create_lore {lore_file_path}"))
                ...
                if validation["is_template"]:
                    ...
                    issues_found.append((lore_file_path, "COMPLETE", "Fill out placeholder sections with actual documentation"))
                    
                elif not validation["is_valid"]:
                    status = "🟠 INCOMPLETE"
                    missing_sections = ", ".join(validation["missing_sections"])
                    issues.append(f"Missing: {missing_sections}")
                    issues_found.append((lore_file_path, "FIX", f"Add missing sections: {missing_sections}"))
                    ...
                    if changed_tracked:
                        ...
                        if lore_relative not in changed_files:
                            ...
                            issues_found.append((lore_file_path, "UPDATE", f"Review changes in: {', '.join(changed_tracked)}"))
                        else:
                            status = "✅ UP TO DATE"
                    else:
                        status = "✅ UP TO DATE"
            ...
            table.add_row(status, f"{lore_root}/{lore_file_path}", tracked_files_str, issues_str)
        ...
        if issues_found:
            ...
            for lore_file, action, description in issues_found:
                ...
                if lore_file in mapping:
                    files_list = ", ".join(mapping[lore_file])
                    console.print(f"    [dim]REVIEW THESE FILES TO UNDERSTAND THE ENTIRE SYSTEM:[/dim]")
