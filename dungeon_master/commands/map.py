    ...
    for i, (name, data) in enumerate(items):
        ...
        if data["type"] == "dir":
            ...
            if "children" in data and data["children"]:
                lines.extend(render_tree_markdown(
                    data["children"], 
                    next_prefix, 
                    is_item_last, 
                    level + 1
...
def run_map():
    ...
    try:
        ...
        if not mapping:
            console.print("  [yellow]No track_lore decorators found - creating basic map[/yellow]")
        else:
        ...
        def add_to_rich_tree(tree_node, data_dict):
            for name, data in data_dict.items():
                if data["type"] == "dir":
                    ...
                    if data.get("lore_files"):
                        lore_list = ", ".join(data["lore_files"])
                        lore_info = f" [italic yellow](tracked by {lore_list})[/italic yellow]"
                    ...
                    if data.get("tracked_files"):
                        tracked_list = ", ".join(data["tracked_files"])
                        tracked_info = f" [italic cyan](tracks {tracked_list})[/italic cyan]"
...
"""
        ...
        console.print(f"  • [bold]{sum(len(files) for files in mapping.values())}[/bold] tracked source files")
